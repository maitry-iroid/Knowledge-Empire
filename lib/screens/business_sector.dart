...
import 'package:ke_employee/BLoC/bloc_provider.dart';
...
//    isSwitched = Global.shared.isInstructionView;
  ...
  Widget build(BuildContext context) {
      ...
      return Container(
        ...
        decoration: BoxDecoration(
            ...
            image: Injector.isBusinessMode
                ? DecorationImage(
                    image:
                        AssetImage(Utils.getAssetsImg("business_sec_header")),
                    fit: BoxFit.fill)
                : null),
        child: Row(
          children: <Widget>[
    ...
    Widget _buildSearchResults(LearningModuleResponse results) {

      print("riddhi____"+results.data.length.toString());
      // 2
      return ListView.separated(
        itemCount: results.data.length,
        separatorBuilder: (BuildContext context, int index) => Divider(),
        itemBuilder: (context, index) {
          final location = results.data[index];
          return showItem(index);
        },
      );
    }

    Widget _buildResults(ModuleBloc bloc) {
